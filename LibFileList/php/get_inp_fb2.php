<?php   
 ...
 <?php 
//--------------------------------------------------------------------------------
function WriteFile($FileName,$Folder,$Net,$Lang)
{
	$sep=chr(4);
    $FileName = str_replace(chr(10),'',$FileName);
	
	$query = 'SELECT `BookId`,`Title`,`FileSize`,`FileType`,`Deleted`,`Time` FROM libbook WHERE BookId='.$FileName.';';
	...
	if ($q_book)
	{
		...
		//          тип файла ;              дата;       язык
		$out.=$sep.$book[3].$sep.$sdate.$sep.$LangStr;
