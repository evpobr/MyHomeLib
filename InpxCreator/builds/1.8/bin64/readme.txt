...
options:
  ...
  --strict              Ignore files with file extension not equal to file type
                        in database
...
Обратите пожалуйста внимание на то, что USR архивы Либрусека в настоящий момент
...
разные архиваторы - я встретил RAR и ZIP. Как я понимаю в настоящий момент 
MyHomeLib такие файлы процессировать не может - т.е. программа вынимает ZIP 
из общего архива и запускает на него viewer, который как правило не может 
показать такой файл.  Чтобы избежать этой ерунды можно воспользоваться 
опцией --strict. Тогда lib2inpx не будет включать в INP файлы, для которых 
...
В предлагаемые архивы входит коммандный файл sync_script.cmd , который принимает 
параметр – путь к локальным архивам librusec: 
“sync_script.cmd d:\librusec\local”.  При выполнении скрипт
...
3. Запустит lib2inpx для FB2 – полученный INPX будет лежать в поддиректории data
4. Запустит lib2inpx для USR – полученный INPX будет лежать в поддиректории data

Если указать в командой строке еше один параметр: 
“sync_script.cmd d:\librusec\local skip” то шаг 1 будeт пропущен.
