...
procedure TMHLCoverPanel.Translate(Lang: integer);
const
  Rus : array [1..7] of string = ('Изд-во:','Город:','Год:','ISBN:','Создано:','Версия:','Автор:');
  ENG : array [1..7] of string = ('Publ-r:','City:','Year:','ISBN:','Created:','Version:','Author:');

var
  I: Integer;
begin
  for I := 0 to FInfo.ComponentCount - 1 do
  if (FInfo.Components[i] is TLabel) and (FInfo.Components[i].Tag > 0) then
  case Lang of
    $409:(FInfo.Components[i] as TLabel).Caption := ENG[FInfo.Components[i].Tag];
    $419:(FInfo.Components[i] as TLabel).Caption := RUS[FInfo.Components[i].Tag];
  end;
end;
