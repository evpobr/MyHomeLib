...
//******************************************************************************
  ...
  ['{BAA9D5DC-9FF4-4382-9BFD-EC9065BD0125}']
    ...
    procedure SetPassword(const password: UnicodeString); stdcall;
    procedure SetProperty(name: UnicodeString; value: OleVariant); stdcall;
    procedure SetClassId(const classid: TGUID);
    function GetClassId: TGUID;
    property ClassId: TGUID read GetClassId write SetClassId;
  end;
...
begin
  case bool of
    true: arch.SetProperty(name, 'ON');
    false: arch.SetProperty(name, 'OFF');
  end;
end;
...
begin
  Arch.SetProperty('M', ZipCompressionMethod[method]);
end;
...
type
  ...
  private
    ...
    FSize: Cardinal;
  public
    property SourceMode: TSourceMode read FSourceMode write FSourceMode;
    property Stream: TStream read FStream write FStream;
    property Attributes: Cardinal read FAttributes write FAttributes;
    property CreationTime: TFileTime read FCreationTime write FCreationTime;
    property LastWriteTime: TFileTime read FLastWriteTime write FLastWriteTime;
    property Path: UnicodeString read FPath write FPath;
    property IsFolder: Boolean read FIsFolder write FIsFolder;
    property IsAnti: Boolean read FIsAnti write FIsAnti;
...
procedure T7zOutArchive.SetProperty(name: UnicodeString;
  value: OleVariant);
var
  intf: ISetProperties;
  p: PWideChar;
